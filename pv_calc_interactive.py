...
def main():
    ...
    # Rango seguro de serie#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Calculadora FV on-grid (interactiva por consola)
...
Entrega:
- kWp requeridos para el objetivo
- Rango S permitido por tensiones con explicación (mín S por MPPT_min en CALOR; máx S por min(Vdc_max inv, Vsys_max mod) en FRÍO y por MPPT_max)
- Chequeos eléctricos (tensión/corriente)
- Producción estimada diaria y mensual
- Recomendaciones automáticas si la producción NO cubre la demanda
- Plan automático de distribución de strings por MPPT
...
def format_checks(checks: Dict[str, bool]) -> str:
    ...
    lines = ["    (OK = cumple, FALLA = fuera de rango)"]
    for k, label in mapping.items():
        status = "OK" if checks.get(k, False) else "FALLA"
        lines.append(f"  - {label}: {status}")
    return "\n".join(lines)
...
def main():
    ...
    HSP = ask_float("HSP [h/día]", 4.0, 0.1)
    PR = ask_float("PR [0-1]", 0.80, 0.1, 1.0)
    T_amb_min = ask_float("Temperatura ambiente mínima [°C] (para FRÍO/Voc/Isc)", 8.0, -50, 60)
    T_cell_hot = ask_float("Temperatura de celda caliente [°C] (para CALOR/Vmp)", 65.0, 25, 90)
    ...
    if ask_yesno("¿Calcular automáticamente el # en serie mínimo para activar el MPPT en CALOR?", default=True):
        rng = suggest_series_range(mod, inv, T_cell_hot, T_amb_min)
        n_series = rng["min_series"]
        print(f"  -> Serie mínimo sugerido (CALOR, usa Vmp_hot a {T_cell_hot:.0f}°C): {n_series}S (rango permitido: {rng['min_series']}S ... {rng['max_series']}S)")
    else:
        n_series = ask_int("Módulos en serie (S)", 3, 1)
    ...
    # Rango seguro de serie con datos para explicar
    rng = suggest_series_range(mod, inv, T_cell_hot, T_amb_min)
    ...
    print("\nInversor: ", inv.name)
    print(f"  Ventana MPPT: {inv.mppt_min_v:.0f}–{inv.mppt_max_v:.0f} V  |  Vdc_max: {inv.vdc_max:.0f} V")
    print(f"  Corrientes por MPPT: Imax={inv.imax_mppt:.2f} A  |  Iscmax={inv.iscmax_mppt:.2f} A  |  MPPTs: {inv.n_mppt}")
    ...
    # --- Parámetros térmicos & cómo se aplican ---
    temps = module_at_temps(mod, T_cell_hot, T_amb_min)
    print("\n— Parámetros térmicos & coeficientes —")
    print(f"  Temperaturas de diseño:")
    print(f"    • CALOR: T_celda = {T_cell_hot:.1f} °C  → se aplica γ (Pmax) para Vmp_hot")
    print(f"    • FRÍO:  T_amb  = {T_amb_min:.1f} °C   → se aplican β (Voc) y α (Isc) para Voc_cold e Isc_cold")
    print("  Coeficientes del módulo (por °C):")
    print(f"    • γ Pmax = {mod.gamma_pmax_pct_per_C:.3g}% (suele ser negativo)")
    print(f"    • β Voc  = {mod.beta_voc_pct_per_C:.3g}% (suele ser negativo)")
    print(f"    • α Isc  = {mod.alpha_isc_pct_per_C:.3g}% (positivo)")
    print("  Resultados a las temperaturas de diseño:")
    print(f"    • Vmp_hot(mód) = {temps['vmp_hot']:.2f} V  (con CALOR)")
    print(f"    • Voc_cold(mód) = {temps['voc_cold']:.2f} V  (con FRÍO)")
    print(f"    • Isc_cold(mód) = {temps['isc_cold']:.2f} A  (con FRÍO)")

    # --- Serie permitida por tensiones con explicación detallada ---
    print("\n--- Serie permitida por tensiones ---")
    print("  Criterio mínimo (CALOR / activación MPPT):")
    print(f"    Se usa Vmp_hot(mód) (por γ y T_celda={T_cell_hot:.0f}°C).")
    print(f"    Mín S = ceil(MPPT_min / Vmp_hot_mód) = ceil({inv.mppt_min_v:.1f} / {rng['vmp_hot_mod']:.2f}) = {rng['min_series']}S")

    print("\n  Criterios máximos:")
    print("    • Por TENSIÓN en FRÍO (Voc):")
    print(f"      Vlimit = min(Vdc_max inversor, Vsys_max módulo) = min({inv.vdc_max:.0f}, {mod.max_system_v:.0f}) = {rng['vlimit_used']:.0f} V")
    print(f"      Máx S_por_tensión = floor(Vlimit / Voc_cold_mód) = floor({rng['vlimit_used']:.0f} / {rng['voc_cold_mod']:.2f}) = {rng['max_series_vlimit']}S")
    print("    • Por ventana MPPT en CALOR:")
    print(f"      Máx S_por_MPPT = floor(MPPT_max / Vmp_hot_mód) = floor({inv.mppt_max_v:.0f} / {rng['vmp_hot_mod']:.2f}) = {rng['max_series_mppt']}S")

    print(f"\n  ⇒ Rango válido de S: {rng['min_series']}S … {rng['max_series']}S")
    ...
    if prod_day + 1e-9 < need_kwh_day:
        ...
        print(f"  • Strings actuales: {strings_totales_actual}  |  Strings requeridos: {required_strings}  |  Adicionales: {strings_adicionales_necesarios}")
